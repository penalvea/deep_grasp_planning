...
import time
import datetime
import read_dataset_2d_cylinders
...
write_objects1="/home/penalvea/cylinders/first"
ready_objects1="/home/penalvea/cylinders/first_ready"
write_objects2="/home/penalvea/cylinders/second"
ready_objects2="/home/penalvea/cylinders/second_ready"
write_objects3="/home/penalvea/cylinders/third"
ready_objects3="/home/penalvea/cylinders/third_ready"
write_objects4="/home/penalvea/cylinders/fourth"
ready_objects4="/home/penalvea/cylinders/fourth_ready"
write_objects5="/home/penalvea/cylinders/fifth"
ready_objects5="/home/penalvea/cylinders/fifth_ready"
write_objects6="/home/penalvea/cylinders/sixth"
ready_objects6="/home/penalvea/cylinders/sixth_ready"
write_objects7="/home/penalvea/cylinders/seventh"
ready_objects7="/home/penalvea/cylinders/seventh_ready"
write_objects8="/home/penalvea/cylinders/eighth"
ready_objects8="/home/penalvea/cylinders/eighth_ready"
write_objects9="/home/penalvea/cylinders/nineth"
ready_objects9="/home/penalvea/cylinders/nineth_ready"
write_objects10="/home/penalvea/cylinders/tenth"
ready_objects10="/home/penalvea/cylinders/tenth_ready"
...
output_path="/home/penalvea/NetResults/cylinders"
...
def inference_4layers(inp):
    ...
    lineal1=denselyConnLayerLineal(dens2, 50, 4, 'lineal1_cylinder')
...
height_error=tf.Variable(0.0, name="epochHeightError")
tf.summary.scalar("epochHeightError", height_error)

radius_error=tf.Variable(0.0, name="epochRaidusError")
tf.summary.scalar("epochRaidusError", radius_error)
...
train_writer=tf.summary.FileWriter("/home/penalvea/tensorboard/cylinder/train", sess.graph)
test_writer=tf.summary.FileWriter("/home/penalvea/tensorboard/cylinder/test")
...
while(now<run_until):
    if change==1:
        if folder==1:
            ...
            [training, validation] = read_dataset_2d_cylinders.readNextDataSet(dataset_path, folder1, height, width, depth)
            ...
            [training, validation] = read_dataset_2d_cylinders.readNextDataSet(dataset_path, folder2, height, width, depth)
            ...
            [training, validation] = read_dataset_2d_cylinders.readNextDataSet(dataset_path, folder3, height, width, depth)
            ...
            [training, validation] = read_dataset_2d_cylinders.readNextDataSet(dataset_path, folder4, height, width, depth)
            ...
            [training, validation] = read_dataset_2d_cylinders.readNextDataSet(dataset_path, folder5, height, width, depth)
            ...
            [training, validation] = read_dataset_2d_cylinders.readNextDataSet(dataset_path, folder6, height, width, depth)
            ...
            [training, validation] = read_dataset_2d_cylinders.readNextDataSet(dataset_path, folder7, height, width, depth)
            ...
            [training, validation] = read_dataset_2d_cylinders.readNextDataSet(dataset_path, folder8, height, width, depth)
            ...
            [training, validation] = read_dataset_2d_cylinders.readNextDataSet(dataset_path, folder9, height, width, depth)
            ...
            [training, validation] = read_dataset_2d_cylinders.readNextDataSet(dataset_path, folder10, height, width, depth)
    ...
    height_acum+=errors[0]
    radius_acum+=errors[1]
    rot_x_acum+=errors[2]
    rot_y_acum+=errors[3]
    ...
    if i>0 and i%int(train_objects/batch_size)==0:
        ...
        sess.run(height_error.assign(height_acum/objects))
        sess.run(radius_error.assign(radius_acum / objects))
        ...
        print ("iteration %d, loss_function: %.6f, height error: %.2f, radius error: %.2f, rot x error: %.2f, rot y error: %.2f, elapsed time: %.2f, iteration time: %.2f" % (i/int(train_objects/batch_size) , acum/objects, height_acum/objects, radius_acum/objects, rot_x_acum/objects, rot_y_acum/objects,  (new_time-init_time)/60,  (new_time-start_time)/60))
        print ((height_acum/objects)*16, (radius_acum/objects)*6, (rot_x_acum/objects)*3.1415, (rot_y_acum/objects)*3.1415)
        ...
        if epochs%10==0:
            acum_test=0
            objects_test=0
            height_acum_test = 0.0
            radius_acum_test = 0.0
            rot_x_acum_test = 0.0
            rot_y_acum_test = 0.0
            ...
            for j in range(int(val_objects/batch_size)):
                ...
                acum_test+=loss_test
                errors = np.sum(np.sqrt(np.power((label - result), 2)), 0)
                height_acum_test += errors[0]
                radius_acum_test += errors[1]
                rot_x_acum_test += errors[2]
                rot_y_acum_test += errors[3]


            sess.run(epoch_loss.assign(acum_test/ objects_test))
            sess.run(height_error.assign(height_acum_test / objects_test))
            sess.run(radius_error.assign(radius_acum_test / objects_test))
